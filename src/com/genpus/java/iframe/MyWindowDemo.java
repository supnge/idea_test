...
class MyWindowDemo {
    ...
    public void init() {
        f = new JFrame("by 星云");// 创建窗体对象
        f.setBounds(300, 100, 800, 600);// 设置窗体位置和大小
        f.setLayout(new FlowLayout());// 设置窗体布局为流式布局

        tf = new JTextField(30);// 创建单行文本对象60长度大小字符

        but = new JButton("转到");// 创建按钮对象

        ta = new JTextArea(20, 40);// 创建多行文本对象25行，70列
